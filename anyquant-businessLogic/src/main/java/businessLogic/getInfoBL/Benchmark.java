...
public class Benchmark implements GetBenchmarkBLService {
	...
	public String analysis() {
		...
		if (close > open) {
			conclusion += "阳线说明买方的力量强过卖方，阳线越长，说明多方力量胜过空方越多，后市继续走强的可能性就越大；";
		}
		if (close < open) {
		...
		if (high == close && low == open) {
			...
			conclusion += "价格冲高回落，涨势受阻，虽然收盘价仍高于开盘价，但上方有阻力，可视为弱势；";

		} else if (high == close && low < open && close > open) {// 阳线带下影线
			conclusion += "价格一度大幅下滑，但受到买盘势力支持，价格又回升向上，收盘在最高价处，属强势形态；";

		} else if (high == open && low < close && close < open) {// 阴线带下影线
			conclusion += "价格一度大幅下滑后但受到买盘势力支持，价格回升向上，" + "虽然收盘价仍然低于开盘价，也可视为强势，但在高价区出现时，" + "说明价格有回调要求，应注意卖出；";
			...
			if (high - close < close - low)
				conclusion += "开盘后价格大幅下滑，但在低位处获得支撑，下方买盘积极主动，" + "最终在最高价附近收盘，属强势，当长下影线出现在低价区时，常常是重要的反转信号；";
		} else if (high == open && open == close && close == low) {// 一字线
		...
		if (yesterday.getHigh() > yesterday.getOpen() && yesterday.getLow() < yesterday.getClose()
			...
			if (yesterday.getHigh() - yesterday.getClose() > yesterday.getClose() - yesterday.getLow()) {
				if (today.getOpen() > yesterday.getClose() && today.getClose() < today.getOpen()) {
					...
					if (b < 0.07) {
						result += " 昨日收上十字线，多头上攻受阻；第二天再度高开低走，最终收盘于昨收盘价附近，" + "表明多空争夺激烈，上方卖压较重，应密切留意后市，注意出货；";
					}
				}
			}
			if (today.getOpen() < yesterday.getClose() && today.getClose() < today.getOpen()) {
				result += "昨日收十字线，价格有反转迹象；第二天即开盘于昨收盘价之下，" + "随后价格一路下滑，最终以阴线报收，表明空方占据主动，多预示行情转跌，应注意出货；";
			}
		}
		...
		if (yesterday.getClose() > yesterday.getOpen()) {
			...
			if (drate > 0.03 && drate < 0.07) {
				if (today.getOpen() > yesterday.getClose() && today.getClose() < yesterday.getClose()) {
					result += "昨日收中阳线，买方气势正盛；第二天高开后多方无力顺势上攻，" + "大幅下滑于昨收盘价之下收盘，于是行情走软；高位盘整后出现此形态，应提防庄家拉高出货";
				}
				if (today.getClose() > yesterday.getClose() && today.getClose() < today.getOpen()) {
					result += "多空交战异常激烈，多头胜一筹，应密切留意后市变化；";
				}
			}
		}
